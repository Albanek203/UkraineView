...
public class RegionRepository : BaseRepository<Region>, IPagination<Region> {
    ...
    public override async Task<IReadOnlyCollection<Region>> GetAllAsync() =>
        await this.Entities.Include(x => x.MapImage).Include(x => x.Images).Include(x => x.Entertainments)
                  .ThenInclude(x => x.WorkTime).Include(x => x.Entertainments).ThenInclude(x => x.Address)
    ...
    public override async Task<IReadOnlyCollection<Region>>
        ...
        await this.Entities.Include(x => x.Entertainments).ThenInclude(x => x.WorkTime).Include(x => x.Entertainments)
